...
This installs the OT-2 edge project and bundled driver:
...
```bash
uv run python -c "from driver import opentron_OT2; print('Opentrons driver ready')"
```
...
```python
opentron_OT2.capture_robot_image(filename=None, captures_folder="captures")
```
...
```bash
uv run python -c "from driver import opentron_OT2; robot = opentron_OT2(robot_ip='<robot-ip-address>'); print(robot.capture_robot_image())"
```

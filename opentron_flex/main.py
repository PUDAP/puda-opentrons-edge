"""
Main entry point for the Opentrons Flex edge service.

This module provides the main event loop for the Flex machine, handling command
...
async def main():
    """Initialize the Flex driver and NATS edge runner."""
    config = load_config()
    logger.info(
    ...
    driver = opentron_flex(
        robot_ip=config.opentrons_ip,
    )
    driver.startup()
    logger.info("Flex machine initialized successfully")

"""High-level Opentrons Flex machine interface.

The :class:`opentron_flex` class is the PUDA machine driver for Flex. Public methods
are exposed as PUDA CLI / NATS commands (primitives and JSON-safe structures
only). Helper methods are private.

PUDA contract methods: ``shutdown``, ``home``, ``reset``, ``get_position``.

Flex protocols follow the Opentrons Python Protocol API
(https://docs.opentrons.com/python-api/): Flex pipettes, coordinate deck
slots, trash bins, and optional gripper moves.
...
class opentron_flex:
    ...
    def __init__(
        ...
        logger.info(
            "Flex driver initialised (ip=%s port=%s)",
            robot_ip, port,
        )
    ...
    def startup(self) -> None:
        """
        Start the Flex driver.

        The Flex HTTP connection is stateless, so startup currently logs the
        target robot URL and does not open persistent resources.
        """
        logger.info("Flex driver started (url=%s)", self._base_url)

    def shutdown(self) -> bool:
        """
        Shutdown the machine. Releases driver resources.

        Used by PUDA CLI ``puda machine shutdown``. The Flex HTTP connection is
        stateless so no robot teardown is required.

        Returns:
            bool: True if the shutdown was successful, False otherwise
        """
        logger.info("Flex driver stopped")
        return True

    def home(self) -> bool:
        """
        Home all Flex robot axes.

        Used by PUDA CLI ``puda machine home <machine_id>``. Calls the robot
        HTTP ``POST /robot/home`` endpoint.

        Returns:
            bool: True if the home was successful, False otherwise
        """
        try:
            resp = self.post("/robot/home", json={"target": "robot"}, timeout=120)
            ok = resp.status_code in (200, 201)
            if ok:
                logger.info("Flex robot homed")
            else:
                logger.error("Home failed (HTTP %s): %s", resp.status_code, resp.text)
            return ok
        except Exception as exc:
            logger.error("Home failed: %s", exc)
            return False
    ...
    def reset(self) -> bool:
        ...
        Used by PUDA CLI ``puda machine reset <machine_id>``. Stops the latest
        non-terminal run if one is active and clears local tip-tracking state.
        ...
        try:
            ...
            run_status = str(status.get("run_status", "")).lower()
            if run_id and run_status not in _TERMINAL_STATES | {"offline", "unknown", ""}:
                self.stop(run_id)
            self._tip_offsets.clear()
            logger.info("Flex driver reset")
            return True
        except Exception as exc:
    ...
    def get_position(self) -> dict:
        """
        Get the current gantry / pipette position from the robot.

        Returns:
            dict: Position data from the robot when available, otherwise
            ``{"x": None, "y": None, "z": None}``.
        """
        try:
            resp = self._get_with_retry("/robot/positions", timeout=5)
            ...
            logger.debug("Could not read Flex position: %s", exc)
    ...
    def is_connected(self) -> bool:
        """
        Check whether the Flex robot is reachable on the network.
    ...
    def capture_robot_image(
        ...
        """
        Capture an image from the Flex camera.
    ...
    def get_pipette_types(self) -> list[str]:
        ...
        Lists Flex pipette load names that can be used in load_instrument
        protocol commands. No robot connection is required.

        Returns:
            list[str]: Pipette instrument names (e.g. "flex_1channel_1000").
        """
    ...
    def get_module_types(self) -> list[str]:
        ...
        Returns:
            list[str]: Module names for load_module protocol commands.
        """
        return get_module_types()

    def get_deck_slots(self) -> list[str]:
        """
        Return Flex coordinate deck slots (A1–D4).

        Returns:
            list[str]: Deck slot labels usable as labware locations.
        """
        return get_deck_slots()
